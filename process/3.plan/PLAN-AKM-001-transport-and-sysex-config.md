...
- **Assignee**: AI
- **Verification**: Re-run in this session, on Windows with MSVC 19.51, CMake 4.3.1 and the Visual Studio 2026 generator. *Test-first:* with only the tests written, a `-DBUILD_TESTS=ON` build compiled Catch2 then failed with `error C1083: Cannot open include file: 'akm/LibraryInfo.hpp'`, and the header-scan test failed (`ctest`: 1 test failed out of 1); once the library existed both passed. *AC 1:* fresh directory `juce/build-win-akm-fresh`, default options, real `FetchContent` of JUCE: configure exit 0, build exit 0, 0 warnings, 0 errors, `xs56k_midi`, `xs56k_midi_juce`, `xs56k_framework` and `xs56k_akm` built, Catch2 not fetched, `BUILD_TESTS:BOOL=OFF`; the compile command of `xs56k_akm` carries `/W4 /WX /permissive-`; `cmake --graphviz` shows `xs56k_akm -> xs56k_midi` and `xs56k_akm -> xs56k_warnings` (interface, private) as its only edges. *AC 2:* clean rebuild with `-DBUILD_TESTS=ON`, Debug and Release: 0 warnings, 0 errors; `ctest -C <config>`: 2 of 2 tests passed in each (the Catch2 smoke test and `akm_public_headers_have_no_juce_include`). *AC 3:* the header-scan test passes and a Grep for an include starting with `juce` under `juce/akm` finds 0 occurrences; the script was also shown to fail on `#include <juce_core/juce_core.h>` and on `  #  include "JuceHeader.h"`, to pass on `#include <string>`, and to refuse an empty directory, so it cannot pass vacuously. *AC 4:* both workflows now configure with `-DBUILD_TESTS=ON` and have a `Test` step running `ctest --test-dir juce/build --output-on-failure`; their YAML parses and `python juce/tools/generate_workflows.py --check` reports 15 generated workflows up to date. After `feature/AKM` was pushed at the owner's request (commit `9f514b6`), `gh run watch` on the `linux-headless-canary` run 36239401937 ended with exit 0, every step green (`Configure`, `Build`, `Test`), and its log shows GNU 11.4.0 and `100% tests passed, 0 tests failed out of 2` (the smoke test and `akm_public_headers_have_no_juce_include`), which is the Linux/GCC build that could not be run locally. **Not verified:** macOS; the deployment canary runs (`windows-x64-*`, `linux-x64-*`, `macos-arm64-*`, which build `xs56k_akm` with `BUILD_TESTS` off) were still in progress when this was closed and were not waited for; the `linux-headless-preprod` workflow, which is identical but triggers only on `dev`, has not run.
...
- **Assignee**: AI
- **Verification**: Re-run in this session on Windows with MSVC 19.51 (CMake 4.3.1, Visual Studio 2026 generator), Debug and Release. *Test-first:* with the five new test files and no codec, the build failed with `error C1083: Cannot open include file` for `akm/Checksum.hpp`, `akm/Confirmation.hpp` and `akm/ByteReader.hpp`; once the codec existed they passed. *Result:* clean rebuild of every target in each configuration: 0 warnings, 0 errors; `ctest -C <config>`: 73 tests, 73 passed in each (71 new Catch2 test cases named after their acceptance criteria, the smoke test of TASK-AKM-003 and the no-JUCE-include check). Covered, as the acceptance criteria name them: the spec's frame `F0 47 5E 05 10 0C 1B 35 6D 59 F7` and its checksums-off tip frame `F0 47 5E 00 00 00 04 00 04 F7`; the word `03 01` = 385; the signed word −37 = `01 00 25` and its round trip; minimum and maximum round trips of every width; a foreign, truncated, unknown-Reply-ID, invalid-data-byte or wrongly checksummed message rejected; no exception on any prefix of four valid frames and on any single-byte replacement (8 values, 3 modes) in three of them; in mode unknown a DONE decoded with and without a valid trailing checksum, and rejected with a wrong extra byte or with two extra bytes. The checksums written in the test frames (`58`, `74`, `6A`, `72`, `63`) were added up by hand, not produced by the code. *Do the tests bite:* six temporary mutations of the code — checksum without the `7F` mask, mode unknown accepting any extra byte, the device byte without the user-ref count, the signed read accepting any sign byte, mode On not verifying the checksum, mode Off stripping a byte — each made 1 to 6 tests fail; the code was restored and the suite re-run green. A Grep for an include starting with `juce` under `juce/akm` finds 0 occurrences. **Not verified:** Linux/GCC (nothing pushed since TASK-AKM-003; the CI compiles with `-Wall -Wextra -Wpedantic -Werror`); the checksum coverage of confirmations and every constructed frame against the real S5000 (RQ-AKM-017).
...
- **Dependencies**: TASK-AKM-003
- **Assignee**: AI
- **Verification**: Re-run in this session on Windows with MSVC 19.51 (CMake 4.3.1, Visual Studio 2026 generator), Debug and Release. Clean rebuild of every target in each configuration: 0 warnings, 0 errors; `ctest -C <config>`: 103 tests, 103 passed in each (30 new Catch2 test cases named after the acceptance criteria, on top of the 73 of TASK-AKM-004). Covered, as the criteria name them: on the manual scheduler, a task at 100 ms runs only after 99 ms then 1 ms; tasks at 200 ms then 100 ms, advanced by 100 ms, run the second only; a task cancelled before its time or from inside another task never runs; a task that schedules another runs it at its own time, and `now()` reads each task's deadline while it runs; on the real scheduler, a task at 50 ms ran exactly once (after a further 150 ms) on another thread, tasks run in deadline order, a cancelled task never ran, a scheduler destroyed with a task an hour ahead returned without running it; on the real executor, three producer threads posting 500 tasks each never overlapped and each kept its order, tasks run on the worker thread (`isCurrentThread()` true there and false outside), a task that posts another runs it behind those already queued, an executor destroyed with a task queued behind a running one discarded it after the running one finished; on the manual executor, nothing runs until `runUntilIdle()`, tasks run in posting order, a task posted from another thread runs on the draining thread. *Do the tests bite:* seven temporary mutations (the manual clock not moving to a task's deadline; cancelled tasks still run, which needs two edits because two checks protect it; the thread executor posting at the front; the thread executor draining its queue on destruction; the manual executor never reporting its thread; the manual scheduler ignoring the current time when scheduling) each made 1 to 2 tests fail, and the code was restored and re-run green. The 11 real-thread tests were run 30 times in a row in Release with no failure. **Not verified:** absence of data races under a race detector (ThreadSanitizer is not available on MSVC; the design keeps every shared member behind one mutex, an atomic or a write-before-publish, and the CI has no sanitizer job); Linux/GCC and macOS (nothing pushed since the codec commit).
